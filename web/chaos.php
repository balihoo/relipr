...
if(preg_match('/^RAND([0-9]+)$/', $chaos, $matches)) {
	$rate = intval($matches[1]);
	if(rand(1, $rate) == 1) {
		$keys = array_keys($errors);
		$chaos = $keys[rand(0, count($errors) - 1)];
	}
}

// If the value in the chaos header is mapped, then just kick it out
if(array_key_exists($chaos, $errors) && is_numeric($chaos) && intval($chaos) == $chaos) {
	$response = new Tonic\Response(intval($chaos), $errors[$chaos]);
	$response->output();
	exit;
}
